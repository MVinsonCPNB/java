...
public class ShoppingCartServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // add product to shopping cart stored in session variable
        if (product != null && quantity >= 0) {
            ShoppingItem shoppingItem = new ShoppingItem(product, quantity);
            // returns current or starts new session
            HttpSession session = request.getSession();
            // returns shopping cart session variable or null.
            List<ShoppingItem> cart = (List<ShoppingItem>) session
                    .getAttribute(CART_ATTRIBUTE);
            if (cart == null) {
                cart = new ArrayList<ShoppingItem>();
                // store a new session variable name="cart", value=cart<object>
